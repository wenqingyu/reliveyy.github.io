...
如果你发先 `hava_ssl` 是 DISABLED，那么请先检查错误日志，我的一次错误经验就是在 CentOS 上面我把证书放在了 `/root` 目录下面，导致无论如何 `hava_ssl` 都是 DISABLED。（原因是 MariaDB 是没有权限访问 `/root` 目录的，所以在 Linux 上面还是尽量不要把一些软件启动需要的文件放在 `/root` 下面，一面出现权限问题）
